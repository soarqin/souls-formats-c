...
static void bxf4_file_free_with_name_pool(sf_binder_file_t *f, const sf_allocator_t *a,
                                          const char *pool, size_t pool_size) {
    if (!f) return;
    if (!bxf4_name_in_pool(f->name_utf8, pool, pool_size)) {
...
static sf_result_t bxf4_bulk_copy_names(sf_binder_file_t *files,
...
void sf_bxf4_destroy(sf_bxf4_t *b) {
    ...
    for (size_t i = 0; i < b->file_count; i++) {
        bxf4_file_free_with_name_pool(&b->files[i], a, b->name_pool, b->name_pool_size);
    }
...
static sf_result_t bxf4_populate_from_readers(sf_bxf4_t           *b,
    ...
    r = bxf4_bulk_copy_names(b->files, headers, n, &b->name_pool,
...
cleanup_files:
    for (size_t i = 0; i < b->file_count; i++) {
        bxf4_file_free_with_name_pool(&b->files[i], b->alloc, b->name_pool,
                                      b->name_pool_size);
    }
...
sf_result_t sf_bxf4_remove_file(sf_bxf4_t *b, size_t index) {
    ...
    bxf4_file_free_with_name_pool(&b->files[index], b->alloc, b->name_pool,
                                  b->name_pool_size);
...
static void bxf4_reader_free(sf_bxf4_reader_t *r) {
    ...
    if (r->files) {
        for (size_t i = 0; i < r->file_count; i++) {
            bxf4_file_free_with_name_pool(&r->files[i], a, r->name_pool, r->name_pool_size);
        }
        sf_xfree(a, r->files);
    }
...
sf_result_t sf_bxf4_reader_open(sf_bxf4_reader_t **out,
    ...
    if (n > 0) {
        ...
        r = bxf4_bulk_copy_names(rd->files, headers, n, &rd->name_pool,
                                 &rd->name_pool_size, a);
        if (r != SF_OK) goto cleanup;
    }
