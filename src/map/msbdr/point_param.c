 ...
 *
 * souls-formats-c — MSBDR legacy MSB PointParam.
 *
 * Mirrors upstream:
 *   SoulsFormats/Formats/MSB/MSBDR/PointParam.cs
 */
...
static bool msbdr_region_shape_type_is_known(uint32_t type) {
    return type <= MSBDR_REGION_SHAPE_BOX;
}

static bool msbdr_region_shape_has_data(msbdr_region_shape_type_t type) {
    return type != MSBDR_REGION_SHAPE_POINT;
}
...
static sf_result_t msbdr_region_read_shape_data(sf_binary_reader_t *r, msbdr_region_t *region) {
    sf_result_t rc;
    switch (region->shape_type) {
    case MSBDR_REGION_SHAPE_CIRCLE:
        return sf_binary_reader_read_f32(r, &region->shape.circle.radius);
    case MSBDR_REGION_SHAPE_SPHERE:
        return sf_binary_reader_read_f32(r, &region->shape.sphere.radius);
    case MSBDR_REGION_SHAPE_CYLINDER:
    ...
    case MSBDR_REGION_SHAPE_RECTANGLE:
    ...
    case MSBDR_REGION_SHAPE_POINT:
    default:
        return SF_ERR_UNSUPPORTED_VERSION;
    }
}

static sf_result_t msbdr_region_write_shape_data(sf_binary_writer_t *w, const msbdr_region_t *region) {
    sf_result_t rc;
    switch (region->shape_type) {
    case MSBDR_REGION_SHAPE_CIRCLE:
...
static sf_result_t msbdr_region_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                        msbdr_region_t *out, const sf_allocator_t *a) {
    if (!r || !out) return SF_ERR_INVALID_ARG;
    memset(out, 0, sizeof(*out));
    out->alloc = a;
    out->type = MSBDR_REGION_LOGIC;
    ...
    if (!msbdr_region_shape_type_is_known(shape_type)) return SF_ERR_UNSUPPORTED_VERSION;
    out->shape_type = (msbdr_region_shape_type_t)shape_type;
    if (name_offset == 0 || unk_offset_a == 0 || unk_offset_b == 0 || entity_data_offset == 0) {
        return SF_ERR_BAD_MAGIC;
    }
    if (msbdr_region_shape_has_data(out->shape_type) != (shape_data_offset != 0)) {
        return SF_ERR_BAD_MAGIC;
    }
    ...
    if (shape_data_offset != 0) {
        ...
        rc = msbdr_region_read_shape_data(r, out); if (rc != SF_OK) return rc;
    }
...
sf_result_t msbdr_point_param_read(sf_binary_reader_t *r, int32_t count, sf_msbdr_t *out,
    ...
    out->regions = (sf_msbdr_region_t *)sf_xalloc(a, bytes);
    ...
    if (rc == SF_OK) {
        for (int32_t i = 0; i < count; i++) {
            rc = msbdr_region_read_one(r, entry_offsets[i], &out->regions[i].data, a);
            if (rc != SF_OK) break;
        }
    }
    ...
    if (rc != SF_OK) {
        msbdr_point_param_free(out->regions, count, a);
        sf_xfree(a, out->regions);
        out->regions = NULL;
        out->region_count = 0;
    }
...
static sf_result_t msbdr_region_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msbdr_region_write_one(sf_binary_writer_t *w, const msbdr_region_t *region,
                                         int32_t id, int32_t index) {
    if (!msbdr_region_shape_type_is_known((uint32_t)region->shape_type)) return SF_ERR_UNSUPPORTED_VERSION;

    char name_key[32], unk_a_key[32], unk_b_key[32], shape_key[32], entity_key[32];
    snprintf(name_key, sizeof name_key, "MsbdrRegionName%d", index);
    snprintf(unk_a_key, sizeof unk_a_key, "MsbdrRegionUnkA%d", index);
    snprintf(unk_b_key, sizeof unk_b_key, "MsbdrRegionUnkB%d", index);
    snprintf(shape_key, sizeof shape_key, "MsbdrRegionShape%d", index);
    snprintf(entity_key, sizeof entity_key, "MsbdrRegionEntity%d", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbdr_region_fill_rel_i32(w, name_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbdr_region_fill_rel_i32(w, unk_a_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbdr_region_fill_rel_i32(w, unk_b_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;

    if (msbdr_region_shape_has_data(region->shape_type)) {
        SF_RESERVE_FILL_PAIR(rc, msbdr_region_fill_rel_i32(w, shape_key, start), return rc);
        rc = msbdr_region_write_shape_data(w, region); if (rc != SF_OK) return rc;
    } else {
        SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_fill_i32(w, shape_key, 0), return rc);
    }

    SF_RESERVE_FILL_PAIR(rc, msbdr_region_fill_rel_i32(w, entity_key, start), return rc);
    return sf_binary_writer_write_i32(w, region->entity_id);
}

static sf_result_t msbdr_region_write_entry(sf_binary_writer_t *w, const void *entry,
                                           size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbdr_region_t *region = (const sf_msbdr_region_t *)entry;
    return msbdr_region_write_one(w, &region->data, (int32_t)index, (int32_t)index);
}

sf_result_t msbdr_point_param_write(sf_binary_writer_t *w, const sf_msbdr_t *msbdr) {
    if (!w || !msbdr || msbdr->region_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "POINT_PARAM_ST", "MsbdrNextList2", msbdr->regions,
                                       (size_t)msbdr->region_count, sizeof(*msbdr->regions),
                                       msbdr_region_write_entry, NULL);
}
