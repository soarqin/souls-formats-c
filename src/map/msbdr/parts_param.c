 ...
 *
 * souls-formats-c — MSBDR legacy MSB PartsParam.
 *
 * Mirrors upstream:
 *   SoulsFormats/Formats/MSB/MSBDR/PartsParam.cs
 */
...
static bool msbdr_part_type_is_known(uint32_t type) {
    switch (type) {
    case MSBDR_PART_MAP_PIECE:
    case MSBDR_PART_OBJECT:
    case MSBDR_PART_ENEMY:
    case MSBDR_PART_PLAYER:
    case MSBDR_PART_COLLISION:
    case MSBDR_PART_NAVMESH:
    case MSBDR_PART_DUMMY_OBJECT:
    case MSBDR_PART_DUMMY_ENEMY:
    case MSBDR_PART_CONNECT_COLLISION:
        return true;
    default:
        return false;
    }
...
static sf_result_t msbdr_part_read_entity_data(sf_binary_reader_t *r, msbdr_part_t *part) {
    sf_result_t rc;
...
static sf_result_t msbdr_part_write_entity_data(sf_binary_writer_t *w, const msbdr_part_t *part) {
    sf_result_t rc;
...
static sf_result_t msbdr_part_read_type_data(sf_binary_reader_t *r, msbdr_part_t *part) {
    sf_result_t rc;
    switch (part->type) {
    case MSBDR_PART_MAP_PIECE:
...
static sf_result_t msbdr_part_write_type_data(sf_binary_writer_t *w, const msbdr_part_t *part) {
    sf_result_t rc;
    switch (part->type) {
    case MSBDR_PART_MAP_PIECE:
...
static sf_result_t msbdr_part_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                      msbdr_part_t *out, const sf_allocator_t *a) {
    ...
    if (!msbdr_part_type_is_known(type)) return SF_ERR_UNSUPPORTED_VERSION;
    out->type = (msbdr_part_type_t)type;
    ...
    rc = msbdr_part_read_entity_data(r, out); if (rc != SF_OK) return rc;
    rc = sf_istream_seek(stream, (int64_t)entry_offset + type_data_offset); if (rc != SF_OK) return rc;
    return msbdr_part_read_type_data(r, out);
}

sf_result_t msbdr_parts_param_read(sf_binary_reader_t *r, int32_t count, sf_msbdr_t *out,
    ...
    out->parts = (sf_msbdr_part_t *)sf_xalloc(a, bytes);
    ...
    if (rc == SF_OK) {
        for (int32_t i = 0; i < count; i++) {
            rc = msbdr_part_read_one(r, entry_offsets[i], &out->parts[i].data, a);
            if (rc != SF_OK) break;
        }
    }
    ...
    if (rc != SF_OK) {
        msbdr_parts_param_free(out->parts, count, a);
        sf_xfree(a, out->parts);
        out->parts = NULL;
        out->part_count = 0;
    }
...
static sf_result_t msbdr_part_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msbdr_part_write_one(sf_binary_writer_t *w, const msbdr_part_t *part,
                                       int32_t id, int32_t index) {
    if (!msbdr_part_type_is_known((uint32_t)part->type)) return SF_ERR_UNSUPPORTED_VERSION;

    char name_key[32], sib_key[32], entity_key[32], type_key[32];
    snprintf(name_key, sizeof name_key, "MsbdrPartName%d", index);
    snprintf(sib_key, sizeof sib_key, "MsbdrPartSib%d", index);
    snprintf(entity_key, sizeof entity_key, "MsbdrPartEntity%d", index);
    snprintf(type_key, sizeof type_key, "MsbdrPartType%d", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbdr_part_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, part->name ? part->name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbdr_part_fill_rel_i32(w, sib_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbdr_part_fill_rel_i32(w, entity_key, start), return rc);
    rc = msbdr_part_write_entity_data(w, part); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbdr_part_fill_rel_i32(w, type_key, start), return rc);
    return msbdr_part_write_type_data(w, part);
}

static sf_result_t msbdr_part_write_entry(sf_binary_writer_t *w, const void *entry,
                                         size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbdr_part_t *part = (const sf_msbdr_part_t *)entry;
    return msbdr_part_write_one(w, &part->data, (int32_t)index, (int32_t)index);
}

sf_result_t msbdr_parts_param_write(sf_binary_writer_t *w, const sf_msbdr_t *msbdr) {
    if (!w || !msbdr || msbdr->part_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "PARTS_PARAM_ST", "MsbdrNextList3", msbdr->parts,
                                       (size_t)msbdr->part_count, sizeof(*msbdr->parts),
                                       msbdr_part_write_entry, NULL);
}
