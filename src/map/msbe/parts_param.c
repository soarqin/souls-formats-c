...
static sf_result_t msbe_part_write_entry(sf_binary_writer_t *w,
                                         const void         *entry,
                                         size_t              index,
                                         void               *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbe_part_t *part = (const sf_msbe_part_t *)entry;
    return msbe_part_write_one(w, &part->data, (int32_t)index, (int32_t)index);
}

sf_result_t msbe_parts_param_write(sf_binary_writer_t *w, const sf_msbe_t *msbe) {
    if (!w || !msbe) return SF_ERR_INVALID_ARG;
    return msb_entry_list_write(w, 73, "PARTS_PARAM_ST", "MsbeNextList5", msbe->parts,
                                (size_t)msbe->part_count, sizeof(*msbe->parts),
                                msbe_part_write_entry, NULL);
}
