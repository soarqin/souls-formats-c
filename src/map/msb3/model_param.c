...
enum { MSB3_MODEL_MAP_PIECE = 0, MSB3_MODEL_OBJECT = 1, MSB3_MODEL_ENEMY = 2,
       MSB3_MODEL_PLAYER = 4, MSB3_MODEL_COLLISION = 5 };

static sf_result_t msb3_model_kind_from_type(uint32_t type, sf_msb_model_kind_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (type) {
    case MSB3_MODEL_MAP_PIECE: *out = SF_MSB_MODEL_MAP_PIECE; return SF_OK;
    case MSB3_MODEL_OBJECT: *out = SF_MSB_MODEL_OBJECT; return SF_OK;
    case MSB3_MODEL_ENEMY: *out = SF_MSB_MODEL_CHARACTER; return SF_OK;
    case MSB3_MODEL_PLAYER: *out = SF_MSB_MODEL_PLAYER; return SF_OK;
    case MSB3_MODEL_COLLISION: *out = SF_MSB_MODEL_COLLISION; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}

static sf_result_t msb3_model_type_from_kind(sf_msb_model_kind_t kind, uint32_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (kind) {
    case SF_MSB_MODEL_MAP_PIECE: *out = MSB3_MODEL_MAP_PIECE; return SF_OK;
    case SF_MSB_MODEL_OBJECT: *out = MSB3_MODEL_OBJECT; return SF_OK;
    case SF_MSB_MODEL_CHARACTER: *out = MSB3_MODEL_ENEMY; return SF_OK;
    case SF_MSB_MODEL_PLAYER: *out = MSB3_MODEL_PLAYER; return SF_OK;
    case SF_MSB_MODEL_COLLISION: *out = MSB3_MODEL_COLLISION; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}
...
static sf_result_t msb3_model_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                       msb3_model_t *out, const sf_allocator_t *a) {
    ...
    rc = sf_binary_reader_assert_i32_one(r, 0); if (rc != SF_OK) return rc;
    (void)id;
    rc = msb3_model_kind_from_type(type, &out->kind); if (rc != SF_OK) return rc;
...
sf_result_t msb3_model_param_read(sf_binary_reader_t *r, int32_t count, sf_msb3_t *out,
    ...
    out->models = (sf_msb3_model_t *)sf_xalloc(a, (size_t)count * sizeof(*out->models));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msb3_model_read_one(r, offsets[i], &out->models[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msb3_model_param_free(out->models, count, a); sf_xfree(a, out->models); out->models = NULL; out->model_count = 0; }
    return rc;
}

static sf_result_t msb3_model_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb3_model_write_entry(sf_binary_writer_t *w, const void *entry,
                                          size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb3_model_t *model = (const sf_msb3_model_t *)entry;
    char name_key[32], sib_key[32];
    snprintf(name_key, sizeof name_key, "Msb3ModelName%zu", index);
    snprintf(sib_key, sizeof sib_key, "Msb3ModelSib%zu", index);
    uint32_t type = 0;
    sf_result_t rc = msb3_model_type_from_kind(model->data.kind, &type); if (rc != SF_OK) return rc;
    ...
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_model_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, model->data.name ? model->data.name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_model_fill_rel_i32(w, sib_key, start), return rc);
...
sf_result_t msb3_model_param_write(sf_binary_writer_t *w, const sf_msb3_t *msb3) {
    if (!w || !msb3 || msb3->model_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "MODEL_PARAM_ST", "Msb3NextList0", msb3->models,
                                       (size_t)msb3->model_count, sizeof(*msb3->models),
                                       msb3_model_write_entry, NULL);
}
