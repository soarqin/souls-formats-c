...
static sf_result_t msb3_event_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                       msb3_event_t *out, const sf_allocator_t *a) {
    ...
    (void)id;
    if (zero != 0 || type != MSB3_EVENT_TREASURE || name_offset == 0 || base_offset == 0 ||
        type_offset == 0) return SF_ERR_BAD_MAGIC;
    out->type = MSB3_EVENT_TREASURE;
    ...
    rc = sf_binary_reader_read_i32(r, &out->treasure_part_index); if (rc != SF_OK) return rc;
    return sf_binary_reader_read_i32(r, &out->item_lot1);
}
...
sf_result_t msb3_event_param_read(sf_binary_reader_t *r, int32_t count, sf_msb3_t *out,
    ...
    out->events = (sf_msb3_event_t *)sf_xalloc(a, (size_t)count * sizeof(*out->events));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msb3_event_read_one(r, offsets[i], &out->events[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msb3_event_param_free(out->events, count, a); sf_xfree(a, out->events); out->events = NULL; out->event_count = 0; }
    return rc;
}

static sf_result_t msb3_event_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb3_event_write_entry(sf_binary_writer_t *w, const void *entry,
                                          size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb3_event_t *event = (const sf_msb3_event_t *)entry;
    if (event->data.type != MSB3_EVENT_TREASURE) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[32], base_key[32], type_key[32];
    snprintf(name_key, sizeof name_key, "Msb3EventName%zu", index);
    snprintf(base_key, sizeof base_key, "Msb3EventBase%zu", index);
    snprintf(type_key, sizeof type_key, "Msb3EventType%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb3_event_fill_rel_i32(w, name_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb3_event_fill_rel_i32(w, base_key, start), return rc);
    ...
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_event_fill_rel_i32(w, type_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, event->data.treasure_part_index); if (rc != SF_OK) return rc;
    return sf_binary_writer_write_i32(w, event->data.item_lot1);
}

sf_result_t msb3_event_param_write(sf_binary_writer_t *w, const sf_msb3_t *msb3) {
    if (!w || !msb3 || msb3->event_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "EVENT_PARAM_ST", "Msb3NextList1", msb3->events,
                                       (size_t)msb3->event_count, sizeof(*msb3->events),
                                       msb3_event_write_entry, NULL);
}
