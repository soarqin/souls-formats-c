...
static sf_result_t msb3_region_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                        msb3_region_t *out, const sf_allocator_t *a) {
    ...
    int32_t base3 = 0, type_offset = 0, unk2c = 0;
    uint32_t type = 0, shape = 0, layer = 0;
    ...
    rc = sf_binary_reader_read_i32(r, &base3); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_read_i32(r, &type_offset); if (rc != SF_OK) return rc;
    (void)id; (void)unk2c; (void)layer;
    if (type != MSB3_REGION_SPAWN_POINT || shape != MSB3_REGION_SHAPE_POINT || name_offset == 0 ||
        base1 == 0 || base2 == 0 || shape_offset != 0 || base3 == 0 || type_offset == 0) {
        return SF_ERR_BAD_MAGIC;
    }
    out->type = MSB3_REGION_SPAWN_POINT;
    out->shape_type = MSB3_REGION_SHAPE_POINT;
    ...
    rc = sf_binary_reader_assert_i16_one(r, 0); if (rc != SF_OK) return rc;
    rc = sf_istream_seek(stream, (int64_t)entry_offset + base3); if (rc != SF_OK) return rc;
    ...
    rc = sf_binary_reader_read_i32(r, &out->type_value0); if (rc != SF_OK) return rc;
...
sf_result_t msb3_point_param_read(sf_binary_reader_t *r, int32_t count, sf_msb3_t *out,
    ...
    out->regions = (sf_msb3_region_t *)sf_xalloc(a, (size_t)count * sizeof(*out->regions));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msb3_region_read_one(r, offsets[i], &out->regions[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msb3_point_param_free(out->regions, count, a); sf_xfree(a, out->regions); out->regions = NULL; out->region_count = 0; }
    return rc;
}

static sf_result_t msb3_region_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb3_region_write_entry(sf_binary_writer_t *w, const void *entry,
                                           size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb3_region_t *region = (const sf_msb3_region_t *)entry;
    if (region->data.type != MSB3_REGION_SPAWN_POINT || region->data.shape_type != MSB3_REGION_SHAPE_POINT) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[32], base1_key[32], base2_key[32], shape_key[32], base3_key[32], type_key[32];
    snprintf(name_key, sizeof name_key, "Msb3RegionName%zu", index);
    snprintf(base1_key, sizeof base1_key, "Msb3RegionBase1%zu", index);
    snprintf(base2_key, sizeof base2_key, "Msb3RegionBase2%zu", index);
    snprintf(shape_key, sizeof shape_key, "Msb3RegionShape%zu", index);
    snprintf(base3_key, sizeof base3_key, "Msb3RegionBase3%zu", index);
    snprintf(type_key, sizeof type_key, "Msb3RegionType%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i32(w, base3_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i32(w, type_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, msb3_region_fill_rel_i32(w, name_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb3_region_fill_rel_i32(w, base1_key, start), return rc);
    rc = sf_binary_writer_write_i16(w, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_pad(w, 4); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_region_fill_rel_i32(w, base2_key, start), return rc);
    rc = sf_binary_writer_write_i16(w, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_pad(w, 4); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_fill_i32(w, shape_key, 0), return rc);
    SF_RESERVE_FILL_PAIR(rc, msb3_region_fill_rel_i32(w, base3_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, region->data.activation_part_index); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, region->data.entity_id); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_region_fill_rel_i32(w, type_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, region->data.type_value0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
...
sf_result_t msb3_point_param_write(sf_binary_writer_t *w, const sf_msb3_t *msb3) {
    if (!w || !msb3 || msb3->region_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "POINT_PARAM_ST", "Msb3NextList2", msb3->regions,
                                       (size_t)msb3->region_count, sizeof(*msb3->regions),
                                       msb3_region_write_entry, NULL);
}
