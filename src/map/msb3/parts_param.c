...
static sf_result_t msb3_part_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                      msb3_part_t *out, const sf_allocator_t *a) {
    ...
    int32_t type_offset = 0, gparam_offset = 0, scene_gparam_offset = 0;
    uint32_t type = 0, layer = 0;
    ...
    rc = sf_binary_reader_read_u32(r, &layer); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_read_u32s(r, 8, out->draw_groups); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_read_u32s(r, 8, out->disp_groups); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_read_u32s(r, 8, out->backread_groups); if (rc != SF_OK) return rc;
    ...
    rc = sf_binary_reader_read_i32(r, &gparam_offset); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_read_i32(r, &scene_gparam_offset); if (rc != SF_OK) return rc;
    (void)id; (void)layer;
    if (type != MSB3_PART_MAP_PIECE || zero != 0 || name_offset == 0 || sib_offset == 0 ||
        entity_offset == 0 || type_offset == 0 || gparam_offset != 0 || scene_gparam_offset != 0) {
        return SF_ERR_BAD_MAGIC;
    }
    out->type = MSB3_PART_MAP_PIECE;
    ...
    rc = sf_binary_reader_read_i32s(r, 8, out->entity_groups); if (rc != SF_OK) return rc;
    rc = sf_binary_reader_assert_i32_one(r, 0); if (rc != SF_OK) return rc;
...
sf_result_t msb3_parts_param_read(sf_binary_reader_t *r, int32_t count, sf_msb3_t *out,
    ...
    out->parts = (sf_msb3_part_t *)sf_xalloc(a, (size_t)count * sizeof(*out->parts));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msb3_part_read_one(r, offsets[i], &out->parts[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msb3_parts_param_free(out->parts, count, a); sf_xfree(a, out->parts); out->parts = NULL; out->part_count = 0; }
    return rc;
}

static sf_result_t msb3_part_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb3_write_default_u32s(sf_binary_writer_t *w, const uint32_t values[8]) {
    uint32_t defaults[8] = { 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu,
                             0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu };
    const uint32_t *data = values ? values : defaults;
    return sf_binary_writer_write_u32s(w, 8, data);
}

static sf_result_t msb3_part_write_entry(sf_binary_writer_t *w, const void *entry,
                                         size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb3_part_t *part = (const sf_msb3_part_t *)entry;
    if (part->data.type != MSB3_PART_MAP_PIECE) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[32], sib_key[32], entity_key[32], type_key[32], gparam_key[32], scene_key[32];
    snprintf(name_key, sizeof name_key, "Msb3PartName%zu", index);
    snprintf(sib_key, sizeof sib_key, "Msb3PartSib%zu", index);
    snprintf(entity_key, sizeof entity_key, "Msb3PartEntity%zu", index);
    snprintf(type_key, sizeof type_key, "Msb3PartType%zu", index);
    snprintf(gparam_key, sizeof gparam_key, "Msb3PartGparam%zu", index);
    snprintf(scene_key, sizeof scene_key, "Msb3PartScene%zu", index);
    ...
    rc = msb3_write_default_u32s(w, part->data.draw_groups); if (rc != SF_OK) return rc;
    rc = msb3_write_default_u32s(w, part->data.disp_groups); if (rc != SF_OK) return rc;
    rc = msb3_write_default_u32s(w, part->data.backread_groups); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    ...
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i32(w, gparam_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i32(w, scene_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, msb3_part_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, part->data.name ? part->data.name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_part_fill_rel_i32(w, sib_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb3_part_fill_rel_i32(w, entity_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, part->data.entity_id); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_pattern(w, 16, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32s(w, 8, part->data.entity_groups); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb3_part_fill_rel_i32(w, type_key, start), return rc);
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_fill_i32(w, gparam_key, 0), return rc);
    return sf_binary_writer_fill_i32(w, scene_key, 0);
}

sf_result_t msb3_parts_param_write(sf_binary_writer_t *w, const sf_msb3_t *msb3) {
    if (!w || !msb3 || msb3->part_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "PARTS_PARAM_ST", "Msb3NextList5", msb3->parts,
                                       (size_t)msb3->part_count, sizeof(*msb3->parts),
                                       msb3_part_write_entry, NULL);
}
