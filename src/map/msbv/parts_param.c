...
static sf_result_t msbv_part_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                        msbv_part_t *out, const sf_allocator_t *a) {
    ...
    (void)id;
    if (type != MSBV_PART_MAP_PIECE || name_offset == 0 || model_offset == 0 ||
        resource_offset == 0 || config_offset == 0) return SF_ERR_BAD_MAGIC;
    out->type = MSBV_PART_MAP_PIECE;
...
sf_result_t msbv_parts_param_read(sf_binary_reader_t *r, int32_t count, sf_msbv_t *out,
    ...
    out->parts = (sf_msbv_part_t *)sf_xalloc(a, (size_t)count * sizeof(*out->parts));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbv_part_read_one(r, offsets[i], &out->parts[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbv_parts_param_free(out->parts, count, a); sf_xfree(a, out->parts); out->parts = NULL; out->part_count = 0; }
    return rc;
}

static sf_result_t msbv_part_fill_rel_i32(sf_binary_writer_t *w, const char *key,
                                            int64_t start) {
...
static sf_result_t msbv_part_write_entry(sf_binary_writer_t *w, const void *entry,
                                           size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbv_part_t *part = (const sf_msbv_part_t *)entry;
    if (part->data.type != MSBV_PART_MAP_PIECE) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[40], model_key[40], resource_key[40], config_key[40];
    snprintf(name_key, sizeof name_key, "MsbvPartName%zu", index);
    snprintf(model_key, sizeof model_key, "MsbvPartModel%zu", index);
    snprintf(resource_key, sizeof resource_key, "MsbvPartRes%zu", index);
    snprintf(config_key, sizeof config_key, "MsbvPartConfig%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbv_part_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, part->data.name ? part->data.name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbv_part_fill_rel_i32(w, model_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, "m00", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbv_part_fill_rel_i32(w, resource_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbv_part_fill_rel_i32(w, config_key, start), return rc);
...
sf_result_t msbv_parts_param_write(sf_binary_writer_t *w, const sf_msbv_t *msb) {
    ...
    return msb_legacy_entry_list_write(w, "PARTS_PARAM_ST", "MsbvNextList5", msb->parts,
                                       (size_t)msb->part_count, sizeof(*msb->parts),
                                       msbv_part_write_entry, NULL);
}
