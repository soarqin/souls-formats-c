...
static sf_result_t msbs_part_write_one(sf_binary_writer_t *w, const msbs_part_t *part,
    ...
    rc = sf_binary_writer_write_i32(w, -1); if (rc != SF_OK) return rc;
    rc = sf_binary_writer_write_i32(w, 0); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i64(w, unk1_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i64(w, unk2_key), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i64(w, gparam_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i64(w, scene_key), return rc);
    SF_RESERVE_FILL_PAIR(rc, sf_binary_writer_reserve_i64(w, unk7_key), return rc);
...
static sf_result_t msbs_part_write_entry(sf_binary_writer_t *w,
                                         const void         *entry,
                                         size_t              index,
                                         void               *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbs_part_t *part = (const sf_msbs_part_t *)entry;
    return msbs_part_write_one(w, &part->data, (int32_t)index, (int32_t)index);
}
...
sf_result_t msbs_parts_param_write(sf_binary_writer_t *w, const sf_msbs_t *msbs) {
    ...
    return msb_entry_list_write(w, 35, "PARTS_PARAM_ST", "MsbsNextList5", msbs->parts,
                                (size_t)msbs->part_count, sizeof(*msbs->parts),
                                msbs_part_write_entry, NULL);
}
