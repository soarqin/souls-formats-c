...
static sf_result_t msbvd_event_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                         msbvd_event_t *out, const sf_allocator_t *a) {
    ...
    (void)id;
    if (type != MSBVD_EVENT_SCRIPT || name_offset == 0 || type_data_offset != 0) return SF_ERR_BAD_MAGIC;
    out->type = MSBVD_EVENT_SCRIPT;
...
sf_result_t msbvd_event_param_read(sf_binary_reader_t *r, int32_t count, sf_msbvd_t *out,
    ...
    out->events = (sf_msbvd_event_t *)sf_xalloc(a, (size_t)count * sizeof(*out->events));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbvd_event_read_one(r, offsets[i], &out->events[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbvd_event_param_free(out->events, count, a); sf_xfree(a, out->events); out->events = NULL; out->event_count = 0; }
    return rc;
}

static sf_result_t msbvd_event_fill_rel_i32(sf_binary_writer_t *w, const char *key,
                                             int64_t start) {
...
static sf_result_t msbvd_event_write_entry(sf_binary_writer_t *w, const void *entry,
                                            size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbvd_event_t *event = (const sf_msbvd_event_t *)entry;
    if (event->data.type != MSBVD_EVENT_SCRIPT) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[40], type_key[40];
    snprintf(name_key, sizeof name_key, "MsbvdEventName%zu", index);
    snprintf(type_key, sizeof type_key, "MsbvdEventType%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbvd_event_fill_rel_i32(w, name_key, start), return rc);
...
sf_result_t msbvd_event_param_write(sf_binary_writer_t *w, const sf_msbvd_t *msb) {
    ...
    return msb_legacy_entry_list_write(w, "EVENT_PARAM_ST", "MsbvdNextList1", msb->events,
                                       (size_t)msb->event_count, sizeof(*msb->events),
                                       msbvd_event_write_entry, NULL);
}
