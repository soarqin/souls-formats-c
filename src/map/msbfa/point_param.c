...
void msbfa_point_param_free(sf_msbfa_region_t *regions, int32_t count,
...
static sf_result_t msbfa_region_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                          msbfa_region_t *out, const sf_allocator_t *a) {
    ...
    (void)id;
    if (type != MSBFA_REGION_SPAWN || shape != MSBFA_REGION_SHAPE_POINT || name_offset == 0 ||
        shape_offset != 0 || point_offset == 0) return SF_ERR_BAD_MAGIC;
    out->type = MSBFA_REGION_SPAWN;
    out->shape_type = MSBFA_REGION_SHAPE_POINT;
...
sf_result_t msbfa_point_param_read(sf_binary_reader_t *r, int32_t count, sf_msbfa_t *out,
    ...
    out->regions = (sf_msbfa_region_t *)sf_xalloc(a, (size_t)count * sizeof(*out->regions));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbfa_region_read_one(r, offsets[i], &out->regions[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbfa_point_param_free(out->regions, count, a); sf_xfree(a, out->regions); out->regions = NULL; out->region_count = 0; }
    return rc;
}

static sf_result_t msbfa_region_fill_rel_i32(sf_binary_writer_t *w, const char *key,
                                              int64_t start) {
...
static sf_result_t msbfa_region_write_entry(sf_binary_writer_t *w, const void *entry,
                                             size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbfa_region_t *region = (const sf_msbfa_region_t *)entry;
    if (region->data.type != MSBFA_REGION_SPAWN ||
        region->data.shape_type != MSBFA_REGION_SHAPE_POINT) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[40], shape_key[40], point_key[40];
    snprintf(name_key, sizeof name_key, "MsbfaRegionName%zu", index);
    snprintf(shape_key, sizeof shape_key, "MsbfaRegionShape%zu", index);
    snprintf(point_key, sizeof point_key, "MsbfaRegionPoint%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbfa_region_fill_rel_i32(w, name_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbfa_region_fill_rel_i32(w, point_key, start), return rc);
...
sf_result_t msbfa_point_param_write(sf_binary_writer_t *w, const sf_msbfa_t *msb) {
    ...
    return msb_legacy_entry_list_write(w, "POINT_PARAM_ST", "MsbfaNextList2", msb->regions,
                                       (size_t)msb->region_count, sizeof(*msb->regions),
                                       msbfa_region_write_entry, NULL);
}
