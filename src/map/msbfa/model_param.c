...
enum { MSBFA_MODEL_MAP_PIECE = 0, MSBFA_MODEL_OBJECT = 1, MSBFA_MODEL_ENEMY = 2,
       MSBFA_MODEL_DUMMY = 4 };

static sf_result_t msbfa_model_kind_from_type(uint32_t type, sf_msb_model_kind_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (type) {
    case MSBFA_MODEL_MAP_PIECE: *out = SF_MSB_MODEL_MAP_PIECE; return SF_OK;
    case MSBFA_MODEL_OBJECT: *out = SF_MSB_MODEL_OBJECT; return SF_OK;
    case MSBFA_MODEL_ENEMY: *out = SF_MSB_MODEL_CHARACTER; return SF_OK;
    case MSBFA_MODEL_DUMMY: *out = SF_MSB_MODEL_PLAYER; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}

static sf_result_t msbfa_model_type_from_kind(sf_msb_model_kind_t kind, uint32_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (kind) {
    case SF_MSB_MODEL_MAP_PIECE: *out = MSBFA_MODEL_MAP_PIECE; return SF_OK;
    case SF_MSB_MODEL_OBJECT: *out = MSBFA_MODEL_OBJECT; return SF_OK;
    case SF_MSB_MODEL_CHARACTER: *out = MSBFA_MODEL_ENEMY; return SF_OK;
    case SF_MSB_MODEL_PLAYER: *out = MSBFA_MODEL_DUMMY; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}
...
static sf_result_t msbfa_model_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                         msbfa_model_t *out, const sf_allocator_t *a) {
    ...
    rc = msbfa_model_kind_from_type(type, &out->kind); if (rc != SF_OK) return rc;
...
sf_result_t msbfa_model_param_read(sf_binary_reader_t *r, int32_t count, sf_msbfa_t *out,
    ...
    out->models = (sf_msbfa_model_t *)sf_xalloc(a, (size_t)count * sizeof(*out->models));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbfa_model_read_one(r, offsets[i], &out->models[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbfa_model_param_free(out->models, count, a); sf_xfree(a, out->models); out->models = NULL; out->model_count = 0; }
    return rc;
}

static sf_result_t msbfa_model_fill_rel_i32(sf_binary_writer_t *w, const char *key,
                                             int64_t start) {
...
static sf_result_t msbfa_model_write_entry(sf_binary_writer_t *w, const void *entry,
                                            size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbfa_model_t *model = (const sf_msbfa_model_t *)entry;
    char name_key[40], resource_key[40], render_key[40], row_key[40];
    snprintf(name_key, sizeof name_key, "MsbfaModelName%zu", index);
    snprintf(resource_key, sizeof resource_key, "MsbfaModelRes%zu", index);
    snprintf(render_key, sizeof render_key, "MsbfaModelRender%zu", index);
    snprintf(row_key, sizeof row_key, "MsbfaModelRow%zu", index);
    uint32_t type = 0;
    sf_result_t rc = msbfa_model_type_from_kind(model->data.kind, &type); if (rc != SF_OK) return rc;
    ...
    SF_RESERVE_FILL_PAIR(rc, msbfa_model_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, model->data.name ? model->data.name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbfa_model_fill_rel_i32(w, resource_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbfa_model_fill_rel_i32(w, render_key, start), return rc);
...
sf_result_t msbfa_model_param_write(sf_binary_writer_t *w, const sf_msbfa_t *msb) {
    ...
    return msb_legacy_entry_list_write(w, "MODEL_PARAM_ST", "MsbfaNextList0", msb->models,
                                       (size_t)msb->model_count, sizeof(*msb->models),
                                       msbfa_model_write_entry, NULL);
}
