...
static sf_result_t msbvi_event_write_entry(sf_binary_writer_t *w,
                                           const void         *entry,
                                           size_t              index,
                                           void               *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbvi_event_t *event = (const sf_msbvi_event_t *)entry;
    return msbvi_event_write_one(w, &event->data, (int32_t)index);
}

sf_result_t msbvi_event_param_write(sf_binary_writer_t *w, const sf_msbvi_t *msbvi) {
    if (!w || !msbvi) return SF_ERR_INVALID_ARG;
    return msb_entry_list_write(w, 52, "EVENT_PARAM_ST", "MsbviNextList1", msbvi->events,
                                (size_t)msbvi->event_count, sizeof(*msbvi->events),
                                msbvi_event_write_entry, NULL);
}
