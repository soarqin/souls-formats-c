...
static sf_result_t msbn_part_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                      msbn_part_t *out, const sf_allocator_t *a) {
    ...
    int32_t name_offset = 0, id = 0, zero = 0;
    uint32_t type = 0;
    ...
    (void)id;
    if (type != MSBN_PART_MAP_PIECE || zero != 0 || name_offset == 0) return SF_ERR_BAD_MAGIC;
    out->type = MSBN_PART_MAP_PIECE;
...
sf_result_t msbn_parts_param_read(sf_binary_reader_t *r, int32_t count, sf_msbn_t *out,
    ...
    out->parts = (sf_msbn_part_t *)sf_xalloc(a, (size_t)count * sizeof(*out->parts));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbn_part_read_one(r, offsets[i], &out->parts[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbn_parts_param_free(out->parts, count, a); sf_xfree(a, out->parts); out->parts = NULL; out->part_count = 0; }
    return rc;
}

static sf_result_t msbn_part_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msbn_part_write_entry(sf_binary_writer_t *w, const void *entry,
                                         size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbn_part_t *part = (const sf_msbn_part_t *)entry;
    if (part->data.type != MSBN_PART_MAP_PIECE) return SF_ERR_UNSUPPORTED_VERSION;
    char name_key[32];
    snprintf(name_key, sizeof name_key, "MsbnPartName%zu", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbn_part_fill_rel_i32(w, name_key, start), return rc);
...
sf_result_t msbn_parts_param_write(sf_binary_writer_t *w, const sf_msbn_t *msbn) {
    if (!w || !msbn || msbn->part_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "PARTS_PARAM_ST", "MsbnNextList1", msbn->parts,
                                       (size_t)msbn->part_count, sizeof(*msbn->parts),
                                       msbn_part_write_entry, NULL);
}
