 ...
 *
 * souls-formats-c — Dark Souls 1 MSB1 ModelParam.
 *
 * Mirrors upstream:
 *   SoulsFormats/Formats/MSB/MSB1/ModelParam.cs
 */
...
enum {
    MSB1_MODEL_TYPE_MAP_PIECE = 0,
    MSB1_MODEL_TYPE_OBJECT    = 1,
    MSB1_MODEL_TYPE_ENEMY     = 2,
    MSB1_MODEL_TYPE_PLAYER    = 4,
    MSB1_MODEL_TYPE_COLLISION = 5,
    MSB1_MODEL_TYPE_NAVMESH   = 6,
};

static sf_result_t msb1_model_kind_from_type(uint32_t type, sf_msb_model_kind_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (type) {
    case MSB1_MODEL_TYPE_MAP_PIECE: *out = SF_MSB_MODEL_MAP_PIECE; return SF_OK;
    case MSB1_MODEL_TYPE_OBJECT:    *out = SF_MSB_MODEL_OBJECT;    return SF_OK;
    case MSB1_MODEL_TYPE_ENEMY:     *out = SF_MSB_MODEL_CHARACTER; return SF_OK;
    case MSB1_MODEL_TYPE_PLAYER:    *out = SF_MSB_MODEL_PLAYER;    return SF_OK;
    case MSB1_MODEL_TYPE_COLLISION: *out = SF_MSB_MODEL_COLLISION; return SF_OK;
    case MSB1_MODEL_TYPE_NAVMESH:   *out = SF_MSB_MODEL_NAVMESH;   return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}

static sf_result_t msb1_model_type_from_kind(sf_msb_model_kind_t kind, uint32_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (kind) {
    case SF_MSB_MODEL_MAP_PIECE: *out = MSB1_MODEL_TYPE_MAP_PIECE; return SF_OK;
    case SF_MSB_MODEL_OBJECT:    *out = MSB1_MODEL_TYPE_OBJECT;    return SF_OK;
    case SF_MSB_MODEL_CHARACTER: *out = MSB1_MODEL_TYPE_ENEMY;     return SF_OK;
    case SF_MSB_MODEL_PLAYER:    *out = MSB1_MODEL_TYPE_PLAYER;    return SF_OK;
    case SF_MSB_MODEL_COLLISION: *out = MSB1_MODEL_TYPE_COLLISION; return SF_OK;
    case SF_MSB_MODEL_NAVMESH:   *out = MSB1_MODEL_TYPE_NAVMESH;   return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}
...
static sf_result_t msb1_model_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                       msb1_model_t *out, const sf_allocator_t *a) {
    ...
    rc = msb1_model_kind_from_type(type, &out->kind);
    if (rc != SF_OK) return rc;
...
sf_result_t msb1_model_param_read(sf_binary_reader_t *r, int32_t count, sf_msb1_t *out,
    ...
    out->models = (sf_msb1_model_t *)sf_xalloc(a, bytes);
    ...
    if (rc == SF_OK) {
        for (int32_t i = 0; i < count; i++) {
            rc = msb1_model_read_one(r, entry_offsets[i], &out->models[i].data, a);
            if (rc != SF_OK) break;
        }
    }
    ...
    if (rc != SF_OK) {
        msb1_model_param_free(out->models, count, a);
        sf_xfree(a, out->models);
        out->models = NULL;
        out->model_count = 0;
    }
...
static sf_result_t msb1_model_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb1_model_write_one(sf_binary_writer_t *w, const msb1_model_t *model,
                                        int32_t id, int32_t index) {
    char name_key[32], sib_key[32];
    snprintf(name_key, sizeof name_key, "Msb1ModelName%d", index);
    snprintf(sib_key, sizeof sib_key, "Msb1ModelSib%d", index);

    uint32_t type = 0;
    sf_result_t rc = msb1_model_type_from_kind(model->kind, &type);
    if (rc != SF_OK) return rc;
    ...
    SF_RESERVE_FILL_PAIR(rc, msb1_model_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, model->name ? model->name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb1_model_fill_rel_i32(w, sib_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, model->sib_path ? model->sib_path : "", true);
    if (rc != SF_OK) return rc;
    return sf_binary_writer_pad(w, 4);
}

static sf_result_t msb1_model_write_entry(sf_binary_writer_t *w, const void *entry,
                                          size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb1_model_t *model = (const sf_msb1_model_t *)entry;
    return msb1_model_write_one(w, &model->data, (int32_t)index, (int32_t)index);
}

sf_result_t msb1_model_param_write(sf_binary_writer_t *w, const sf_msb1_t *msb1) {
    if (!w || !msb1 || msb1->model_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "MODEL_PARAM_ST", "Msb1NextList0", msb1->models,
                                       (size_t)msb1->model_count, sizeof(*msb1->models),
                                       msb1_model_write_entry, NULL);
}
