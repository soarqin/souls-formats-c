 ...
 *
 * souls-formats-c — Dark Souls 1 MSB1 PartsParam.
 *
 * Mirrors upstream:
 *   SoulsFormats/Formats/MSB/MSB1/PartsParam.cs
 */
...
static bool msb1_part_type_is_known(uint32_t type) {
    switch (type) {
    case MSB1_PART_MAP_PIECE:
    case MSB1_PART_OBJECT:
    case MSB1_PART_ENEMY:
    case MSB1_PART_PLAYER:
    case MSB1_PART_COLLISION:
    case MSB1_PART_NAVMESH:
    case MSB1_PART_DUMMY_OBJECT:
    case MSB1_PART_DUMMY_ENEMY:
    case MSB1_PART_CONNECT_COLLISION:
        return true;
    default:
        return false;
    }
...
static sf_result_t msb1_part_read_entity_data(sf_binary_reader_t *r, msb1_part_t *part) {
    sf_result_t rc;
...
static sf_result_t msb1_part_write_entity_data(sf_binary_writer_t *w, const msb1_part_t *part) {
    sf_result_t rc;
...
static sf_result_t msb1_part_read_type_data(sf_binary_reader_t *r, msb1_part_t *part) {
    sf_result_t rc;
    switch (part->type) {
    case MSB1_PART_MAP_PIECE:
...
static sf_result_t msb1_part_write_type_data(sf_binary_writer_t *w, const msb1_part_t *part) {
    sf_result_t rc;
    switch (part->type) {
    case MSB1_PART_MAP_PIECE:
...
static sf_result_t msb1_part_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                      msb1_part_t *out, const sf_allocator_t *a) {
    ...
    if (!msb1_part_type_is_known(type)) return SF_ERR_UNSUPPORTED_VERSION;
    out->type = (msb1_part_type_t)type;
    ...
    rc = msb1_part_read_entity_data(r, out); if (rc != SF_OK) return rc;
    rc = sf_istream_seek(stream, (int64_t)entry_offset + type_data_offset); if (rc != SF_OK) return rc;
    return msb1_part_read_type_data(r, out);
}
...
sf_result_t msb1_parts_param_read(sf_binary_reader_t *r, int32_t count, sf_msb1_t *out,
    ...
    out->parts = (sf_msb1_part_t *)sf_xalloc(a, bytes);
    ...
    if (rc == SF_OK) {
        for (int32_t i = 0; i < count; i++) {
            rc = msb1_part_read_one(r, entry_offsets[i], &out->parts[i].data, a);
            if (rc != SF_OK) break;
        }
    }
    ...
    if (rc != SF_OK) {
        msb1_parts_param_free(out->parts, count, a);
        sf_xfree(a, out->parts);
        out->parts = NULL;
        out->part_count = 0;
    }
...
static sf_result_t msb1_part_fill_rel_i32(sf_binary_writer_t *w, const char *key, int64_t start) {
...
static sf_result_t msb1_part_write_one(sf_binary_writer_t *w, const msb1_part_t *part,
                                       int32_t id, int32_t index) {
    if (!msb1_part_type_is_known((uint32_t)part->type)) return SF_ERR_UNSUPPORTED_VERSION;

    char name_key[32], sib_key[32], entity_key[32], type_key[32];
    snprintf(name_key, sizeof name_key, "Msb1PartName%d", index);
    snprintf(sib_key, sizeof sib_key, "Msb1PartSib%d", index);
    snprintf(entity_key, sizeof entity_key, "Msb1PartEntity%d", index);
    snprintf(type_key, sizeof type_key, "Msb1PartType%d", index);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb1_part_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, part->name ? part->name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb1_part_fill_rel_i32(w, sib_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msb1_part_fill_rel_i32(w, entity_key, start), return rc);
    rc = msb1_part_write_entity_data(w, part); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msb1_part_fill_rel_i32(w, type_key, start), return rc);
    return msb1_part_write_type_data(w, part);
}

static sf_result_t msb1_part_write_entry(sf_binary_writer_t *w, const void *entry,
                                         size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msb1_part_t *part = (const sf_msb1_part_t *)entry;
    return msb1_part_write_one(w, &part->data, (int32_t)index, (int32_t)index);
}

sf_result_t msb1_parts_param_write(sf_binary_writer_t *w, const sf_msb1_t *msb1) {
    if (!w || !msb1 || msb1->part_count < 0) return SF_ERR_INVALID_ARG;
    return msb_legacy_entry_list_write(w, "PARTS_PARAM_ST", "Msb1NextList3", msb1->parts,
                                       (size_t)msb1->part_count, sizeof(*msb1->parts),
                                       msb1_part_write_entry, NULL);
}
