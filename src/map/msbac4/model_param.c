...
enum { MSBAC4_MODEL_MAP_PIECE = 0, MSBAC4_MODEL_OBJECT = 1, MSBAC4_MODEL_ENEMY = 2,
       MSBAC4_MODEL_DUMMY = 4 };

static sf_result_t msbac4_model_kind_from_type(uint32_t type, sf_msb_model_kind_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (type) {
    case MSBAC4_MODEL_MAP_PIECE: *out = SF_MSB_MODEL_MAP_PIECE; return SF_OK;
    case MSBAC4_MODEL_OBJECT: *out = SF_MSB_MODEL_OBJECT; return SF_OK;
    case MSBAC4_MODEL_ENEMY: *out = SF_MSB_MODEL_CHARACTER; return SF_OK;
    case MSBAC4_MODEL_DUMMY: *out = SF_MSB_MODEL_PLAYER; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}

static sf_result_t msbac4_model_type_from_kind(sf_msb_model_kind_t kind, uint32_t *out) {
    if (!out) return SF_ERR_INVALID_ARG;
    switch (kind) {
    case SF_MSB_MODEL_MAP_PIECE: *out = MSBAC4_MODEL_MAP_PIECE; return SF_OK;
    case SF_MSB_MODEL_OBJECT: *out = MSBAC4_MODEL_OBJECT; return SF_OK;
    case SF_MSB_MODEL_CHARACTER: *out = MSBAC4_MODEL_ENEMY; return SF_OK;
    case SF_MSB_MODEL_PLAYER: *out = MSBAC4_MODEL_DUMMY; return SF_OK;
    default: return SF_ERR_UNSUPPORTED_VERSION;
    }
}
...
static sf_result_t msbac4_model_read_one(sf_binary_reader_t *r, int32_t entry_offset,
                                         msbac4_model_t *out, const sf_allocator_t *a) {
    ...
    rc = msbac4_model_kind_from_type(type, &out->kind); if (rc != SF_OK) return rc;
...
sf_result_t msbac4_model_param_read(sf_binary_reader_t *r, int32_t count, sf_msbac4_t *out,
    ...
    out->models = (sf_msbac4_model_t *)sf_xalloc(a, (size_t)count * sizeof(*out->models));
    ...
    for (int32_t i = 0; rc == SF_OK && i < count; i++) rc = msbac4_model_read_one(r, offsets[i], &out->models[i].data, a);
    sf_xfree(a, offsets);
    if (rc != SF_OK) { msbac4_model_param_free(out->models, count, a); sf_xfree(a, out->models); out->models = NULL; out->model_count = 0; }
    return rc;
}

static sf_result_t msbac4_model_fill_rel_i32(sf_binary_writer_t *w, const char *key,
                                             int64_t start) {
...
static sf_result_t msbac4_model_write_entry(sf_binary_writer_t *w, const void *entry,
                                            size_t index, void *ctx) {
    (void)ctx;
    if (index > (size_t)INT32_MAX) return SF_ERR_OUT_OF_RANGE;
    const sf_msbac4_model_t *model = (const sf_msbac4_model_t *)entry;
    char name_key[40], resource_key[40], render_key[40], row_key[40];
    snprintf(name_key, sizeof name_key, "Msbac4ModelName%zu", index);
    snprintf(resource_key, sizeof resource_key, "Msbac4ModelRes%zu", index);
    snprintf(render_key, sizeof render_key, "Msbac4ModelRender%zu", index);
    snprintf(row_key, sizeof row_key, "Msbac4ModelRow%zu", index);
    uint32_t type = 0;
    sf_result_t rc = msbac4_model_type_from_kind(model->data.kind, &type); if (rc != SF_OK) return rc;
    ...
    SF_RESERVE_FILL_PAIR(rc, msbac4_model_fill_rel_i32(w, name_key, start), return rc);
    rc = sf_binary_writer_write_ascii(w, model->data.name ? model->data.name : "", true); if (rc != SF_OK) return rc;
    SF_RESERVE_FILL_PAIR(rc, msbac4_model_fill_rel_i32(w, resource_key, start), return rc);
    ...
    SF_RESERVE_FILL_PAIR(rc, msbac4_model_fill_rel_i32(w, render_key, start), return rc);
...
sf_result_t msbac4_model_param_write(sf_binary_writer_t *w, const sf_msbac4_t *msb) {
    ...
    return msb_legacy_entry_list_write(w, "MODEL_PARAM_ST", "Msbac4NextList0", msb->models,
                                       (size_t)msb->model_count, sizeof(*msb->models),
                                       msbac4_model_write_entry, NULL);
}
