...
2026-05-11: PARAMDEF XML reader uses mxml v4.0.4 vendored as a static library (`mxml_static`). The mxml autoconf build is bypassed; we generate `config.h` with `HAVE_PTHREAD_H` undefined and compile the 9 core `.c` files ourselves under `cmake/deps/mxml.cmake`. mxml is warning-noisy under `-Wall -Wextra`; isolate it via `target_compile_options(mxml_static PRIVATE -w)` so it doesn't break `-Werror` in the main lib.
2026-05-11: `mxmlFindElement(tree, tree, "X", ...)` calls `mxmlWalkNext` first, so it can never return `tree` itself. When the XML document has no `<?xml ?>` declaration, the loaded tree IS the root element, not a wrapper. Always check `mxmlGetElement(tree)` against the expected root name before falling back to `mxmlFindElement` with `MXML_DESCEND_ALL`.
2026-05-11: PARAMDEF struct layouts (sf_paramdef, sf_paramdef_field) are shared between `paramdef.c` (binary read/write) and `paramdef_xml_read.c` (XML reader) via a new `src/param/paramdef_internal.h` header. Follow the existing pattern used by `param_internal.h` for cross-module struct sharing.
2026-05-11: Def attribute parsing (`"type name"`, `"type name:N"`, `"type name[N]"`, `"type name = default"`) is implemented as a hand-written state machine in `sfi_parse_def_attr`, not regex. Scan backwards from end of the Def string: locate `=` first for the default value, then `[]` for arrays or `:` for bit fields, finally take everything between type and the suffix as the internal name.
2026-05-11: EMEVD writer must set `big_endian`/`varint_long` only after emitting the four flag bytes, then use `ReserveVarint` for header offsets. Bloodborne/DS3 pre-Sekiro instruction arg offsets are the upstream mixed-width quirk: reserve/fill only the low i32 and emit a following i32 zero so the reader's 64-bit varint sees the same value.
2026-05-11: EMEVD layer offsets are relative to the layers section and deduplicated by layer mask in first-seen event/instruction order before filling instruction layer reservations.
