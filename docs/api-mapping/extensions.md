...
20: | `sf_reverse_bits_u8` | `sf_io.h` | Bit-reversal helper for binder format flags (upstream uses bit-big-endian reads) | stable | `include/souls_formats/sf_io.h` |
21: | `sf_bhd5_open` | `sf_bhd5.h` | Two-path API to open .bhd and .bdt together; upstream `BHD5.Read` takes a stream and punts on the .bdt | stable | `include/souls_formats/sf_bhd5.h` |
