 ...
 *
 * Post-v1 Wave 5 (T14) — BTAB e2e: probe every available game archive
 * (ER, AC6, Nightreign, Sekiro) for a .btab entry, parse it through the
 * production sf_btab_read_from_memory path, and verify the
 * write→read round-trip preserves entry count and long-format flag.
 *
...
static const char *const k_er_paths[] = {
    "/map/mapstudio/m60_42_36_00.btab.dcx",
    "/map/mapstudio/m10_00_00_00.btab.dcx",
    "/map/m60/m60_42_36_00/m60_42_36_00.btab.dcx",
    NULL,
};

static const char *const k_ac6_paths[] = {
    "/map/mapstudio/m01_00_00_00.btab.dcx",
    "/map/mapstudio/m10_00_00_00.btab.dcx",
    NULL,
};

static const char *const k_nr_paths[] = {
    "/map/mapstudio/m60_10_09_02.btab.dcx",
    "/map/mapstudio/m21_20_00_00.btab.dcx",
    NULL,
};

static const char *const k_sekiro_paths[] = {
    "/map/mapstudio/m10_00_00_00.btab.dcx",
    "/map/mapstudio/m11_00_00_00.btab.dcx",
    NULL,
};

static void test_btab_e2e_multi_game(void)
{
    ...
    if (!used_path) {
        TEST_IGNORE_MESSAGE(
            "btab e2e: no .btab entries found in Data0 across ER/AC6/NR/"
            "Sekiro (Wave-0 probe confirmed 0; BTAB files likely live in "
            "non-Data0 shards or under unmatched paths)");
    }
    (void)used_game;

    sf_btab_t  *obj = NULL;
    sf_result_t r   = sf_btab_read_from_memory(&obj, bytes, size, NULL);
    sf_free(NULL, bytes);
    TEST_ASSERT_EQUAL_INT(SF_OK, r);
    TEST_ASSERT_NOT_NULL(obj);

    const size_t orig_count = sf_btab_entry_count(obj);
    const bool   orig_long  = sf_btab_is_long_format(obj);

    void  *out_bytes = NULL;
    size_t out_size  = 0;
    r = sf_btab_write_to_buffer(obj, &out_bytes, &out_size, NULL);
    TEST_ASSERT_EQUAL_INT(SF_OK, r);
    TEST_ASSERT_NOT_NULL(out_bytes);
    TEST_ASSERT_GREATER_THAN((size_t)0, out_size);

    sf_btab_t *obj2 = NULL;
    r = sf_btab_read_from_memory(&obj2, out_bytes, out_size, NULL);
    sf_free(NULL, out_bytes);
    TEST_ASSERT_EQUAL_INT(SF_OK, r);
    TEST_ASSERT_NOT_NULL(obj2);

    TEST_ASSERT_EQUAL_size_t(orig_count, sf_btab_entry_count(obj2));
    TEST_ASSERT_EQUAL_INT(orig_long ? 1 : 0,
                          sf_btab_is_long_format(obj2) ? 1 : 0);

    sf_btab_destroy(obj);
    sf_btab_destroy(obj2);
}

int main(void)
{
    UNITY_BEGIN();
    RUN_TEST(test_btab_e2e_multi_game);
    return UNITY_END();
}
