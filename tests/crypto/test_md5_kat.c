...
typedef struct md5_case { const char *msg; uint8_t digest[16]; } md5_case_t;

static const md5_case_t k_cases[] = {
...
static void test_md5_vectors(void) {
    ...
    for (size_t i = 0; i < sizeof(k_cases) / sizeof(k_cases[0]); i++) {
        TEST_ASSERT_EQUAL_INT(SF_OK, sfi_md5_hash(k_cases[i].msg, strlen(k_cases[i].msg), out));
        TEST_ASSERT_EQUAL_UINT8_ARRAY(k_cases[i].digest, out, 16);
    }
}
