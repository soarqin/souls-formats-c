...
static void test_msbvi_route_param_empty_roundtrip(void) {
    sf_msbvi_t msbvi;
    ...
    TEST_ASSERT_EQUAL_INT32(0, sf_msbvi_route_count(read_msbvi));
    TEST_ASSERT_NULL(sf_msbvi_route_at(read_msbvi, 0));
    sf_msbvi_destroy(read_msbvi);
    sf_free(NULL, data);
}

int main(void) { UNITY_BEGIN(); RUN_TEST(test_msbvi_route_param_empty_roundtrip); return UNITY_END(); }
