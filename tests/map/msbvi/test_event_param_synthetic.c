...
static void test_msbvi_event_param_empty_roundtrip(void) {
    sf_msbvi_t msbvi;
    ...
    TEST_ASSERT_EQUAL_INT32(0, sf_msbvi_event_count(read_msbvi));
    TEST_ASSERT_NULL(sf_msbvi_event_at(read_msbvi, 0));
    sf_msbvi_destroy(read_msbvi);
    sf_free(NULL, data);
}

int main(void) { UNITY_BEGIN(); RUN_TEST(test_msbvi_event_param_empty_roundtrip); return UNITY_END(); }
