...
#include "map/msbfa/msbfa_internal.h"
...
static void test_msbfa_synthetic_roundtrip(void) {
    sf_msbfa_model_t models[1];
    sf_msbfa_event_t events[1];
    sf_msbfa_region_t regions[1];
    sf_msbfa_part_t parts[1];
    ...
    events[0].data.type = MSBFA_EVENT_SCRIPT;
    events[0].data.name = (char *)"m0000";
    events[0].data.unique_id = 10;

    regions[0].data.type = MSBFA_REGION_SPAWN;
    regions[0].data.shape_type = MSBFA_REGION_SHAPE_POINT;
    ...
    parts[0].data.type = MSBFA_PART_MAP_PIECE;
    ...
    uint8_t *data = NULL;
    size_t size = 0;
    TEST_ASSERT_EQUAL(SF_OK, sf_msbfa_write_to_memory(&msb, &data, &size, NULL));

    sf_msbfa_t *read_msb = NULL;
    TEST_ASSERT_EQUAL(SF_OK, sf_msbfa_read_from_memory(&read_msb, data, size, NULL));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbfa_model_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbfa_event_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbfa_region_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbfa_part_count(read_msb));

    const sf_msbfa_model_t *read_model = sf_msbfa_model_at(read_msb, 0);
    const sf_msbfa_event_t *read_event = sf_msbfa_event_at(read_msb, 0);
    const sf_msbfa_region_t *read_region = sf_msbfa_region_at(read_msb, 0);
    const sf_msbfa_part_t *read_part = sf_msbfa_part_at(read_msb, 0);
    ...
    TEST_ASSERT_EQUAL(MSBFA_EVENT_SCRIPT, read_event->data.type);
    TEST_ASSERT_EQUAL_INT32(10, read_event->data.unique_id);
    TEST_ASSERT_EQUAL(MSBFA_REGION_SHAPE_POINT, read_region->data.shape_type);
    ...
    sf_msbfa_destroy(read_msb);
    sf_free(NULL, data);
}

int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_msbfa_synthetic_roundtrip);
    return UNITY_END();
}
