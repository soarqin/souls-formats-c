...
#include "map/msbvd/msbvd_internal.h"
...
static void test_msbvd_synthetic_roundtrip(void) {
    sf_msbvd_model_t models[1];
    sf_msbvd_event_t events[1];
    sf_msbvd_region_t regions[1];
    sf_msbvd_part_t parts[1];
    ...
    events[0].data.type = MSBVD_EVENT_SCRIPT;
    events[0].data.name = (char *)"m0000";
    events[0].data.unique_id = 10;

    regions[0].data.type = MSBVD_REGION_SPAWN;
    regions[0].data.shape_type = MSBVD_REGION_SHAPE_POINT;
    ...
    parts[0].data.type = MSBVD_PART_MAP_PIECE;
    ...
    uint8_t *data = NULL;
    size_t size = 0;
    TEST_ASSERT_EQUAL(SF_OK, sf_msbvd_write_to_memory(&msb, &data, &size, NULL));

    sf_msbvd_t *read_msb = NULL;
    TEST_ASSERT_EQUAL(SF_OK, sf_msbvd_read_from_memory(&read_msb, data, size, NULL));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbvd_model_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbvd_event_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbvd_region_count(read_msb));
    TEST_ASSERT_EQUAL_INT32(1, sf_msbvd_part_count(read_msb));

    const sf_msbvd_model_t *read_model = sf_msbvd_model_at(read_msb, 0);
    const sf_msbvd_event_t *read_event = sf_msbvd_event_at(read_msb, 0);
    const sf_msbvd_region_t *read_region = sf_msbvd_region_at(read_msb, 0);
    const sf_msbvd_part_t *read_part = sf_msbvd_part_at(read_msb, 0);
    ...
    TEST_ASSERT_EQUAL(MSBVD_EVENT_SCRIPT, read_event->data.type);
    TEST_ASSERT_EQUAL_INT32(10, read_event->data.unique_id);
    TEST_ASSERT_EQUAL(MSBVD_REGION_SHAPE_POINT, read_region->data.shape_type);
    ...
    sf_msbvd_destroy(read_msb);
    sf_free(NULL, data);
}

int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_msbvd_synthetic_roundtrip);
    return UNITY_END();
}
