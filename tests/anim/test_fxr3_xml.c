...
static void exercise_xml_round_trip(const uint8_t *in, size_t in_size,
    ...
    /* Step 2: write to XML */
    char *xml_str = NULL; size_t xml_size = 0;
    TEST_ASSERT_EQUAL_INT(SF_OK, sf_fxr3_to_xml(f1, &xml_str, &xml_size, NULL));
    TEST_ASSERT_NOT_NULL(xml_str);
    TEST_ASSERT_TRUE(xml_size > 0);
    
    /* Verify XML contains expected strings */
    TEST_ASSERT_NOT_NULL(strstr(xml_str, "<FXR3"));
    TEST_ASSERT_NOT_NULL(strstr(xml_str, "<Container"));

    /* Step 3: read from XML */
    sf_fxr3_t *f2 = NULL;
    TEST_ASSERT_EQUAL_INT(SF_OK, sf_fxr3_from_xml(&f2, xml_str, xml_size, NULL));
    verify(f2);

    /* Step 4: write again to XML */
    char *xml_str2 = NULL; size_t xml_size2 = 0;
    TEST_ASSERT_EQUAL_INT(SF_OK, sf_fxr3_to_xml(f2, &xml_str2, &xml_size2, NULL));
    TEST_ASSERT_NOT_NULL(xml_str2);
    
    /* Step 5: compare XML strings */
    /* Note: structural equality is required, but since we use the same writer,
       the strings should be identical. */
    TEST_ASSERT_EQUAL_STRING(xml_str, xml_str2);
...
static void test_fxr3_ds3_round_trip(void) {
    ...
    exercise_xml_round_trip(b.bytes, sz, verify_ds3);
}
...
static void test_fxr3_sekiro_round_trip(void) {
    ...
    TEST_ASSERT_TRUE(sz > 0u);
    TEST_ASSERT_TRUE(sz <= BUF_CAP);
    exercise_xml_round_trip(b.bytes, sz, verify_sekiro);
}
